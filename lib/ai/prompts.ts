...
{
  "insight_text": "<2-4 short paragraphs written in markdown. Use **bold** for key habit names, goal titles, or the single most important action. Use *italics* for energy framing, emotional tone, or the 'why'. Use <<highlighted text>> (double angle brackets) to call out specific data points — numbers, percentages, streaks, or short key phrases that deserve visual emphasis (e.g. <<5.3/10 energy>>, <<declining trend>>, <<86% habit rate>>). Separate distinct thoughts with a blank line (paragraph break). Use emojis sparingly — max 1 per paragraph, only when they add meaning. No bullet lists, no headers. Must reference specific goals, habits, or energy patterns by name. Should feel like it was written knowing this exact person today — not a generic observation.>",
  "priorities": [
    {
